...
def setup_config_and_paths(args):
    ...
    if not os.path.exists(args.output_dir + os.sep + 'model_predictions'):
        os.mkdir(args.output_dir + os.sep + 'model_predictions')

    if not os.path.exists(args.output_dir + os.sep + 'checkpoint'):
        os.mkdir(args.output_dir + os.sep + 'checkpoint')
    ...
    # Will use zero-shot fitness predictions as an additional model covariate (won't be predicted by the model)
    elif args.augmentation=="zero_shot_fitness_predictions_covariate":
...
def main(args):
    ...
    if not os.path.exists(args.model_location+os.sep+model_name):
        os.mkdir(args.model_location+os.sep+model_name)
    
    with open(args.model_location+os.sep+model_name+os.sep+'training_arguments', 'w') as f:
        json.dump(args.__dict__, f, indent=2)
    
    print(f"Model name: {model_name}")
    print(f"Sequence embeddings: {args.sequence_embeddings_location}")
    ############################# MODEL OUTDIR SETUP #################################      
    ...
    if num_targets==1:
        ...
        assay_file_names={ target_names[0]: assay_file_name }
        if "zero_shot_fitness_predictions" in target_names_input:
            assay_file_names["zero_shot_fitness_predictions"] = assay_file_name
    else:
    ...
    perf_list = [
        ...
        args.model_type,
        args.model_name_suffix,"_".join(target_names_input),
        assay_id,
        args.fold_variable_name,
        trainer_final_status['total_training_steps'],
        trainer_final_status['total_training_epochs'],
        model_hypers_str
    ]
    test_logs, spearmans = log_performance_fold(args, target_names, test_eval_results, trainer_final_status, perf_list, logs_folder=args.output_scores_location)
